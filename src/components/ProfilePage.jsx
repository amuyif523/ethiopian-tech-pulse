...
// Reusable component for displaying a repository
const RepoCard = ({ repo }) => (
  <a href={repo.html_url} target="_blank" rel="noopener noreferrer" className="block p-4 rounded-lg transition-colors duration-200 hover:bg-white/5 border border-white/10">
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Two API requests: one for user details, one for repos
        ...
        // Sort repos by stars and take the top 5
    ...
    // Cleanup function to cancel requests if user navigates away
        ...
        <>
          ...
          <header className="flex flex-col md:flex-row items-center gap-8 mb-12">
            ...
            <div className="text-center md:text-left">
              <h1 className="text-4xl font-extrabold text-white">{profile.details.name}</h1>
              <h2 className="text-2xl font-light text-gray-400">@{profile.details.login}</h2>
